...
In einer dritten Runde der größte zurückgestellte Punkt nachgeholt:
- **Sync-Engine auf Delta/Cursor umgebaut** (`services/pwa/syncEngine.ts` komplett neu, plus neue `*UpdatedSince()`/`*CreatedSince()`-Methoden in `services/storage.legacy.ts`): Statt bei jedem Sync-Zyklus alle Tabellen komplett neu zu laden (Architektur-/Performance-Fund #4/#1) wird pro Nutzer ein Cursor (`sync_cursor:<userId>` in `meta`) gespeichert; jede Tabelle liefert nur noch Zeilen, die seit dem letzten Sync neu/geändert sind. Tastings und der Occasion-Wine-Pool werden jetzt in je einer Abfrage für alle Weine/Anlässe des Nutzers geholt statt einer Abfrage pro Wein/Anlass (N+1 behoben). `inventory_events` (Trinkhistorie) ist jetzt ebenfalls per Cursor begrenzt statt unbegrenzt wachsend bei jedem Zyklus komplett gezogen zu werden.
  - **Hard-Delete-Problem gelöst, ohne eine Tombstone-Tabelle einzuführen**: Ein Delta-Pull sieht nur geänderte Zeilen, nie tatsächlich gelöschte (z. B. endgültiges Löschen aus dem Papierkorb). Dafür holt `getSyncReconciliationIds()` einmal pro Zyklus nur die `id`-Spalte aller Weine/Anlässe/Instanzen/Pool-Einträge (winzige Payload) und der Client entfernt lokal, was serverseitig nicht mehr existiert.
  - **Sicherheits-Guard gegen Datenverlust**: Diese Reconciliation läuft nur, wenn die lokale Schreib-Queue für den Nutzer leer ist - sonst könnte ein offline neu angelegter, noch nicht synchronisierter Wein fälschlich als "serverseitig gelöscht" erkannt und lokal gelöscht werden, bevor er je hochgeladen wurde. Mit Test abgesichert (`tests/unit/pwa.syncEngine.test.ts`).
  - Neue Migration `20260722000033_add_occasion_wine_pool_updated_at.sql`: `occasion_wine_pool` hatte nur `created_at`, obwohl Einträge in-place aktualisiert werden können (Priorität/Flaschenzahl) - ohne `updated_at` hätte der Cursor solche Änderungen verpasst.
  - `cellar_pockets` bewusst nicht umgestellt (bleibt Voll-Pull): kleine, selten geänderte Tabelle ohne Lösch-Pfad, ein Delta-Mechanismus dafür wäre Overengineering.

Weiterhin **nicht** umgesetzt (bewusst zurückgestellt): Feature-Page-Zerlegung, echtes Erfassungsformular mit Struktur-Slidern - beides UI-Großrefactorings, die eher eine eigene fokussierte Sitzung verdienen als einen weiteren Anhang an diese.
